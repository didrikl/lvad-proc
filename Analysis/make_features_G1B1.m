function F = make_features_G1B1(sequences, Data, partSpecNo, idSpecs, accVars, noteVars)
	...
	for i=1:nSeqs
		...
		Notes = Data.(sequences{i}).Notes;
		T = Data.(sequences{i}).Plot_Data.T{partSpecNo};
		t = Data.(sequences{i}).Plot_Data.RPM_Order_Map{partSpecNo}.time;
		...
		for j=1:numel(accVars)
			var = accVars{j};
			mags = Data.(sequences{i}).Plot_Data.RPM_Order_Map{partSpecNo}.([var,'_mags']);
