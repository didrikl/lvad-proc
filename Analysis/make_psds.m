function [psds,f,pows] = make_psds(S_analysis,vars,id_specs)
    ...
    for j=1:numel(seqs)
        ...
        for k=1:numel(speeds)
            multiWaitbar('Making spectral densities','Increment',(k/(numel(speeds)*numel(seqs))));        
        ...
        psds{j} = join(psds{j},id_specs(:,{'analysis_id','idLabel'}),'Keys','analysis_id');  
