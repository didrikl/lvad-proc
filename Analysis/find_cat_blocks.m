...
function blocks = find_cat_val_blocks(T,blocks)
    ...
    for i=1:numel(block.start_times)
        ...
        start_ind = find(T.dur==block.start_times(i));
        if start_ind==height(T), disp hei; end
        if isempty(start_ind)
            blocks.start_inds(i) = 1;
        else
            blocks.start_inds(i) = start_ind;
        end
        %         blocks.end_inds(i)
        %         block.end_times(i)
        
        %if i>numel(block.end_time)
        try
        end_ind = find(T.dur==block.end_times(i));
        if isempty(end_ind)
            blocks.end_inds(i) = height(T);
        else
            blocks.end_inds(i) = end_ind;
        end
