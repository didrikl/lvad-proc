function h_figs = plot_control_interventions(F,G,vars,type)

markers = {'o','diamond','square','pentagram'};
speeds = {'2200','2500','2800','3100'};
QReds = unique(G.QRedTarget_pst);

% Use nominal as different baselines
...
for v=1:size(vars,1)
    var = vars{v,1};
    title_str = ['Plot 8: ',type,' change in acc against Q reduction at all speeds'];
    ...
    for i=1:numel(catheters)
		...
		F_c_inds = interventionType=='Control' | ...
		...
		for j=1:numel(speeds)
            ...
            F_x = double(string(F.QRedTarget_pst(F_c_inds & F_rpm_inds)))+mod(j,2)*0.1-0.1;
            F_y = F.(var)(F_c_inds & F_rpm_inds);
            G_y = G.(var)(G_c_inds & G_rpm_inds);
            G_x = double(string(G.QRedTarget_pst(G_c_inds & G_rpm_inds)))+mod(j,2)*0.1-0.1;
            G_x(isnan(G_x)) = 0;
            F_x(isnan(F_x)) = 0;
