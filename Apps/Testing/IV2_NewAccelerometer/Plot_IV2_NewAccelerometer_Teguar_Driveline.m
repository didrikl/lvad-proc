...
for i=1:numel(parts)
    ...
    tic
    [T,rpms] = make_plot_data(parts{i},S_CB_DL_parts,rpm{i},sampleRate,bl_part,cbl_part);
    toc
    h_fig = plot_ordermap_with_vars(...
        T,orderMapVar,sampleRate,bl_part,mapColScale,notes,circ_ylim);
    fig_name = sprintf('Spectrogram - %s - NewAccelerometer - Teguar - Driveline - RPM=%s - No catheter - %s',...
        orderMapVar,mat2str(rpms));
    set(h_fig,'Name',fig_name);
    save_figure([save_path,'\Figures'], fig_name, 300)
...
function [T,rpm] = make_plot_data(parts,S_parts,rpm,fs,bl_part,cbl_part)
    ...
    for k=1:height(blocks)
        ...
        T.accB_x_rms(range) = rms(T.accB_x(range));
        T.accB_x_std(range) = std(T.accB_x(range));
        T.accB_y_std(range) = std(T.accB_y(range));
        T.accB_z_std(range) = std(T.accB_z(range));
        T.accB_norm_std(range) = std(T.accB_norm(range));
        T.accB_norm_rms(range) = rms(T.accB_norm(range));
        
        freqx{k} = meanfreq(detrend(T.accB_x(range)),fs);
        T.accB_x_mpf(range) = freqx{k};
        T.accB_x_mpf_shift(range) = freqx{k} - freqx{1};
        freqy{k} = meanfreq(detrend(T.accB_y(range)),fs);
        T.accB_y_mpf(range) = freqy{k};
        T.accB_y_mpf_shift(range) = freqy{k} - freqy{1};
        freqz{k} = meanfreq(detrend(T.accB_z(range)),fs);
        T.accB_z_mpf(range) = freqz{k};
        T.accB_z_mpf_shift(range) = freqz{k} - freqz{1};
        freq{k} = meanfreq(detrend(T.accB_norm(range)),fs);
        T.accB_norm_mpf(range) = freq{k};
        ...
        T.accB_norm_std_shift = 100*(T.accB_norm_std-mean(T.accB_norm_std(bl_inds)))/mean(T.accB_norm_std(bl_inds));
        T.accB_norm_movStd_shift = 100*(T.accB_norm_movStd-mean(T.accB_norm_movStd(bl_inds),'omitnan'))/mean(T.accB_norm_movStd(bl_inds),'omitnan');
        T.accB_x_std_shift = 100*(T.accB_x_std-mean(T.accB_x_std(bl_inds)))/mean(T.accB_x_std(bl_inds));
        T.accB_x_movStd_shift = 100*(T.accB_x_movStd-mean(T.accB_x_movStd(bl_inds),'omitnan'))/mean(T.accB_x_movStd(bl_inds),'omitnan');
    
        T.accB_norm_rms_shift = -100*(T.accB_norm_rms-mean(T.accB_norm_rms(bl_inds)))/mean(T.accB_norm_rms(bl_inds));
        T.accB_norm_movRMS_shift = -100*(T.accB_norm_movRMS-mean(T.accB_norm_movRMS(bl_inds),'omitnan'))/mean(T.accB_norm_movRMS(bl_inds),'omitnan');
        T.accB_x_rms_shift = -100*(T.accB_x_std-mean(T.accB_x_rms(bl_inds)))/mean(T.accB_x_rms(bl_inds));
        T.accB_x_movRMS_shift = -100*(T.accB_x_movRMS-mean(T.accB_x_movRMS(bl_inds),'omitnan'))/mean(T.accB_x_movRMS(bl_inds),'omitnan');
...
function add_freqStats(h,T)
    ...
    plot(T.t(ss_rows),T.accB_norm_mpf_shift(ss_rows),...
        'LineWidth',1.5,...
        'LineStyle',':',...
        'Color',[0 0 0,0.8],...
        'HandleVisibility','off');
    T.accB_norm_mpf_shift(not(ss_rows)) = nan;
    plot(T.t,T.accB_norm_mpf_shift,...
        'LineWidth',2,...
        'LineStyle','-',...
    ...
    plot(T.t(ss_rows),T.accB_x_mpf_shift(ss_rows),...
        'LineWidth',1.5,...
        'LineStyle',':',...
        'Color',[0.39,0.56,0.15,0.7],... 
        'HandleVisibility','off');
    T.accB_x_mpf_shift(not(ss_rows)) = nan;
    plot(T.t,T.accB_x_mpf_shift,...
        'LineWidth',2,...
        'LineStyle','-',...
        'DisplayName','MPF_x',...
        'Color',[0.39,0.56,0.15,0.9]);

%     plot(T.t(ss_rows),T.accB_z_mpf_shift(ss_rows),...
%         'LineWidth',1.5,...
%         'LineStyle',':',...
%         'HandleVisibility','off');...,'Color',[0.39,0.56,0.15,0.7]);
%     T.accB_z_mpf_shift(not(ss_rows)) = nan;
%     plot(T.t,T.accB_z_mpf_shift,...
%         'LineWidth',2,...
%         'LineStyle','-',...
...
function add_vibrations(h,T)
    ...
    plot(T.t,T.accB_x_movStd,...
        'LineWidth',0.5,...
        'LineStyle','-',...
        'DisplayName','SD_x, moving 5sec',...
        'Color',[0.30,0.50,0.88,0.3]);
    accB_x_std_ss = T.accB_x_std;
    accB_x_std_ss(not(ss_rows)) = nan;
    plot(T.t,accB_x_std_ss,....
        'LineWidth',2,...
        'LineStyle','-',...
...
%     plot(T.t,T.accB_z_movStd,...
%         'LineWidth',0.5,...
%         'LineStyle','-',...
%         'DisplayName','SD_z, moving 1sec',...
%         'Color',[0.46,0.66,0.79,0.3]);
%     accB_z_std_ss = T.accB_z_std;
%     accB_z_std_ss(not(ss_rows)) = nan;
%     plot(T.t,accB_z_std_ss,....
%         'LineWidth',2,...
%         'LineStyle','-',...
...
%    add_ylabel('SD_z  (g)',specs);
    ...
    plot(T.t,T.accB_norm_movStd,...
        'LineWidth',.5,...
        'LineStyle','-',...
        'DisplayName','SD_{|x,y,z|}, moving 5sec',...
        'Color',[0.96,0.39,0.35,0.65]);
    accB_norm_std_ss = T.accB_norm_std;
    accB_norm_std_ss(not(ss_rows)) = nan;
    plot(T.t,accB_norm_std_ss,....
        'LineWidth',2,...
        'LineStyle','-',...
...
%     accB_x_std_shift_ss = T.accB_x_std_shift;
%     accB_x_std_shift_ss(not(ss_rows)) = nan;
% %     plot(T.t,T.accB_x_movStd_shift,...
% %         'LineWidth',0.5,...
% %         'LineStyle','-',...
% %         'Color',[0.96,0.39,0.35,0.3],...
% %         'DisplayName','SD*, moving 5sec');
%     plot(T.t,accB_x_std_shift_ss,...
%         'LineWidth',2,...
%         'LineStyle','-',...
%         'Color',[0.74,0.04,0.17],...
%         'DisplayName','SD*_x, steady-state');
%     plot(T.t(ss_rows),accB_x_std_shift_ss(ss_rows),...
%         'LineWidth',1.25,...
%         'LineStyle',':',...
%         'Color',[0.74,0.04,0.17,0.6],...[0.7188,0.6289,0.9297,0.7],...[0.6055,0.1406, 0.4414,0.6],...
%         'HandleVisibility','off');
    
%     accB_norm_std_shift_ss = T.accB_norm_std_shift;
%     accB_norm_std_shift_ss(not(ss_rows)) = nan;
% %     plot(T.t,T.accB_norm_movStd_shift,...
% %         'LineWidth',0.5,...
% %         'LineStyle','-',...
% %         'Color',[0.74,0.04,0.17,0.05],...
% %         'DisplayName','SD*, moving 5sec');
%     plot(T.t,accB_norm_std_shift_ss,...
%         'LineWidth',2,...
%         'LineStyle','-',...
%         'Color',[0.74,0.04,0.17],...
%         'DisplayName','-SD_{|\it\bfa\rm|}');
%     plot(T.t(ss_rows),accB_norm_std_shift_ss(ss_rows),...
%         'LineWidth',1.25,...
%         'LineStyle',':',...
%         'Color',[0.74,0.04,0.17,0.6],...[0.7188,0.6289,0.9297,0.7],...[0.6055,0.1406, 0.4414,0.6],...
%         'HandleVisibility','off');
    ...
    plot(T.t,T.accB_norm_movStd_shift,...
        ...
        'Color',[0.74,0.04,0.17,0.05],...
        'HandleVisibility','off');
    T.accB_norm_movStd_shift(not(ss_rows)) = nan;
    plot(T.t,T.accB_norm_movStd_shift,...
        'LineWidth',1,...
        'LineStyle','-',...
