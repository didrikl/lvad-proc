...
for i=1:numel(parts)
    ...
    tic
    [T,rpms] = make_plot_data(parts{i},S_CB_parts,rpm{i},sampleRate,bl_part,cbl_part);
    toc
    h_fig = plot_ordermap_with_vars(...
        T,orderMapVar,sampleRate,bl_part,mapColScale,notes,circ_ylim);
    fig_name = sprintf('Spectrogram - %s - NewAccelerometer - Teguar - LVAD - RPM=%s - No catheter - %s',...
