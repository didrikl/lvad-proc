...
mapSpec = {
    % Variable       Colorbar   y-lims
    'accA_norm',    [0,20], [0,5.2];
    'accA_x',       [0,20], [0,5.2];
    'accA_y',       [0,20], [0,5.2];
    'accA_z',       [0,20], [0,5.2];
    'accB_norm',    [-20,20], [0,5.2];
    'accB_x',       [-20,20], [0,5.2];
    'accB_y',       [-20,20], [0,5.2];
    'accB_z',       [-20,20], [0,5.2];
    'i_norm',       [-20,20], [0,2.3];
...
parts = {
%     [],   [2],  [],   '01. RPM changes'
%     [],   [8],  [],   '01. RPM changes - Second time'
%     [],   [3],  [],   '02. Graft clamping'
%      {},   [4],   [],   '03. Balloon inflation'
%      {},   [6],   [],   '04. Balloon inflation'
%     [],   [7],   [],   '05. Balloon inflation'
%    [],   [9],  [],   'Saline bolus injections'
%    [],   [9],  [],   'Saline bolus injections'
%{8,103}, 9:11, [],   'Thrombus injections 1-3'
            %{11,133}, 12:14, [],   'Thrombus injections 4-6'
            {14,143}, 15:18, [], ''
            };
...
function [h_fig,map,order] = plot_ordermap_with_vars(...
    ...
    T.t = seconds(T.time-T.time(1))+map_time(1);
     ...
     T_ref = T(T.noteRow==143,:);
     [map_ref,order_ref,rpm_ref,map_time_ref] = make_rpm_order_map(T_ref,orderMapVar,fs,...
        'pumpSpeed', 0.02, 90); %
    spec_ref = orderspectrum(map_ref,order_ref,'Amplitude','peak');
    map = map+spec_ref;
    ref_end_ind = find(map_time>map_time_ref(end),1,'first');
    
    ref_diff_map = map(:,1:ref_end_ind);
    map(map<prctile(ref_diff_map(:),60))=min(map(:));
