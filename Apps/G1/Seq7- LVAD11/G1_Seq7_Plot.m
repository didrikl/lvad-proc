...
S_parts_filt = S_parts;
for i=1:numel(parts)
    welcome(['Part(s) ',num2str(parts{i})],'iteration')
   
    T = merge_table_blocks(S_parts(parts{i}));
     T.dur = linspace(0,1/700*height(T),height(T))';
    y = T.accA_norm;%(height(T)-700*150:height(T));
    x = T.dur;%(height(T)-700*150:height(T));
%    y(y>3)=NaN;
    
%     [sp,values,rho] = spaps(double(x),double(y),8);
%     [points] = fnplt(sp);
    x2 = points(1,:)';
    y2 = points(2,:)';
    T2 = timetable(x2,y2,'RowTimes',seconds(x2));
    T.time=seconds(T.dur);
    T3 = synchronize(T,T2);    
    T3.Properties.SampleRate = 700;
    %T3.accA_norm = T3.accA_norm-T3.y2;
% 
     T3.accA_norm = highpass(T3.accA_norm,0.11,'Steepness',0.85,'StopbandAttenuation',60);
     T3.accA_norm = lowpass(T3.accA_norm,0.37,'Steepness',0.85,'StopbandAttenuation',60);
     T3 = calc_moving(T3,'accA_norm',{'Std'},700*15);
          
    [T3,rpms] = make_plot_data2(T3,rpm{i},sampleRate,bl_part,cbl_part);
     h_fig = plot_ordermap_with_vars(...
         T3,orderMapVar,sampleRate,bl_part,mapColScale,notes,circ_ylim,mapOrderLim);
