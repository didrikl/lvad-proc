...
% orderMapVar = {'accA_norm_[[1 2 4]]hFilt'};%,'accB_norm'};
% mapColScale = [-70,0];
%orderMapVar = {'accA_x','accA_y','accA_z'};
...
part_label = {
    'No catheter'
    'Sensor3InDAC2'
    'Sensor3InDAC2_4.5x20mmTREK'
    '4.5x20mmTREK'
    'xySwapped'
    'Sensor4InTeguar_4.5mmx20mmTREK'
    'Sensor4InTeguar_4.0mmx20mmRAIDEN'
    'Sensor4InTeguar_4.0mmx20mmPAPYRUS'
    'Sensor3Turned90Deg'
    'RigOnFloor_Sensor4Monuted90DegOnLVAD'
    'RigOnFloor_4.0mmx20mmPAPYRUS_LessWire'
    'RigOnFloor_4.0mmx20mmPAPYRUS_WireHalfway'
    'RigOnFloor_6.0mmx8mm_LessWire'
    'RigOnFloor_6.0mmx8mm_WireHalfway'
    'RigOnFloor_NoTubClip'
    'RigOnFloor_NoTubClip_6.0mmx8mm'
    'RigOnFloor_NoTubClip_6.0mmx20mm'
    };
...
for i=1:numel(parts)
    ...
    for j=1:numel(orderMapVar)
        h_fig = plot_ordermap_with_vars(...
            T,orderMapVar{j},sampleRate,bl_part,mapColScale,notes,circ_ylim);
        fig_name = sprintf('Part %d - PowerLab - %s - RPM=%s - %s',...
            parts{i},orderMapVar{j},mat2str(rpms),part_label{parts{i}});
...
function add_freqStats(h,T)
    ...
    plot(T.t,T.accA_x_mpf_shift,...
        ...
        'LineStyle','-',...
        'DisplayName','MPF_x',...
        'Color',[0.72,0.85,0.55,0.9]);

    plot(T.t(ss_rows),T.accA_z_mpf_shift(ss_rows),...
        'LineWidth',1.5,...
        'LineStyle',':',...
        'HandleVisibility','off',...
        'Color',[0.76,0.54,0.90,0.7]);
    T.accA_y_mpf_shift(not(ss_rows)) = nan;
    plot(T.t,T.accA_y_mpf_shift,...
        'LineWidth',1.5,...
        'LineStyle','-',...
        'DisplayName','MPF_y',...
        'Color',[0.76,0.54,0.90,0.9]);
    
    plot(T.t(ss_rows),T.accA_z_mpf_shift(ss_rows),...
        'LineWidth',1.5,...
        'LineStyle',':',...
        'HandleVisibility','off',...
        'Color',[0.94,0.66,0.55,0.7]);
    T.accA_z_mpf_shift(not(ss_rows)) = nan;
    plot(T.t,T.accA_z_mpf_shift,...
        'LineWidth',1.5,...
        'LineStyle','-',...
        'DisplayName','MPF_z',...
        'Color',[0.94,0.66,0.55,0.9]);
