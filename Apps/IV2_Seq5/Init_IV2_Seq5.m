...
sequence = 'IV2_Seq5';
experiment_subdir = [sequence,' - Water simulated HVAD thrombosis'];
...
PL_seq5 = init_powerlab_raw_matfiles(powerlab_filePaths,'',powerlab_variable_map);

% Read meassured flow and emboli (volume and count) from M3 ultrasound
US_seq5 = init_m3_raw_textfile(ultrasound_filePaths);

% Read driveline accelerometer data
%DL_seq5 = init_cardibox_raw_txtfile(driveline_filePaths,'','accC');

% Read sequence notes made with Excel file template
notes_seq5 = init_notes_xlsfile_ver3_9(notes_filePath);
...
feats = init_features_from_notes(notes_seq5);

clear S
%S = fuse_data_parfor(notes,PL,US);
S = fuse_data(notes_seq5,PL_seq5,US_seq5);
%clear PL US
