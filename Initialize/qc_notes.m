function notes = qc_notes(notes, idSpecs, askToReInit, notesType)
...
function misNum = check_missing_numeric_data(notes, numVarsNotToCheck)
	...
	misNum = any(isnan(notes{:,vartype('numeric')}),2) & ...
...
function irregID = check_analysis_ids(Notes, idSpecs)
	...
	analyse_events = {'-','Echo on','Echo','Echo off','Injection, embolus'};
	...
	if height(idSpecs)>0
		...
		for i=1:height(idSpecs)
			...
			idNote_inds = Notes.analysis_id==id;
