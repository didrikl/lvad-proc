...
%   Author: Navan Ruthramoorthy
  ...
  methods (Access = 'private')
    ...
    function createToolbar(this)
      ...
      [cd,m,alpha] = imread('C:\Users\Didrik\Dropbox\Arbeid\OUS\Proc\Matlab\Tools\External\audioeditor\audioeditor\icons\ae_zoom_select.png');
      cd = double(cd) / 255;
      cd(~alpha) = NaN;
      ...
      [cd,m,alpha] = imread('C:\Users\Didrik\Dropbox\Arbeid\OUS\Proc\Matlab\Tools\External\audioeditor\audioeditor\icons\ae_zoom_outfull.png');
      cd = double(cd) / 255;
      cd(~alpha) = NaN;
      uipushtool(audiotb, 'CData', cd, ...
