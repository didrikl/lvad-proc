function isOK = check_id_parameter_consistency_IV2(S,idSpecs)
    ...
    if ismember('balLev',S.Properties.VariableNames)
		idSpecs.balLev = categorical(idSpecs.balLev);
		idSpecs.balLev(idSpecs.balLev=='0') = '-';
        idSpecs.balLev(isundefined(idSpecs.balLev)) = '-';
		
        balLevInData = unique(S.balLev);		
        notListedBalloon = not(ismember(balLevInData,idSpecs.balLev));
        if ismissing(balLevInData), balLevInData = '<Missing>'; end
		if any(notListedBalloon)
            msg = sprintf(['Unlisted balloon diameters found.',...
                '\n\tID: %s',...
                '\n\tBalloon diameter in ID spec.: %s',...
                '\n\tBalloon diameter in data: %s'],...
                char(idSpecs.analysis_id),string(char(idSpecs.balLev)),...
