function lead_signal = sync_lead_with_lvad_acc(lead_signal, lvad_signal)
    ...
    % finer adjustments  
    ts_start_diff = lead_signal.time(1) - lvad_signal.time(1);
    lead_signal.time = lead_signal.time - ts_start_diff;
    ...
    lead_signal.time = lead_signal.time - ts_data_diff;
    
    % Verify visually that syncing worked (at picked manually event/window, 
    % that not containing the sync points)
    lvad_zoom  = lvad_signal.time > '07-Dec-2018 17:00:30.011' & lvad_signal.time < '07-Dec-2018 17:05:10.523';
    lead_zoom  = lead_signal.time > '07-Dec-2018 17:00:30.011' & lead_signal.time < '07-Dec-2018 17:05:10.523';
    close all
    figure('Name','3 component vector length for LVAD accelerometer')
    h1 = plot(lvad_signal.time(lvad_zoom),lvad_signal.accNorm(lvad_zoom));
    ylim([0.7 1.3]);
    figure('Name','3 component vector length for driveline accelerometer')
    h2 = plot(lead_signal.time(lead_zoom),lead_signal.accNorm(lead_zoom));
    ylim([0.7 1.3]);
    
    % Verify visually also with movrms
    figure('Name','Moving RMS for LVAD accelerometer')
    plot(lvad_signal.time,lvad_signal.acc_movRMS)
    figure('Name','Moving RMS for driveline accelerometer')
    plot(lead_signal.time,lead_signal.acc_movRMS)
