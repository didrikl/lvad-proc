function hFig = make_all_injection_figures_ver2(T, Notes, map, var, Config, partSpec)
	...
	hPlt = plot_curves(hSub(1), T, yLims);
	
	add_shades(hSub(3), segs, segs.all.isEcho, echoShadeColor, 0.20, [1.0*yLims(1) 200])
	add_shades(hSub(3), segs, segs.all.isTransitional, shadeColor, 0.30, [1.0*yLims(1) 200])
	add_shades(hSub(3), segs, segs.all.isWashout, washoutColor, 0.25, [59 63])
	add_shades(hSub(3), segs, segs.all.isTransitional & segs.all.isInjection, injectionColor, 0.25, [59 63])
		
	make_xticks_and_time_str(hSub, segs);
	add_xlines_and_grid(hSub);
	add_transition_annotations(hSub(3), T, segs);
 	add_steady_state_annotations(hSub(3), segs);
...
% 	h3Avg(not(isfinite(h3Avg))) = 0;
	h(1) = plot(hAx, t, movmedian(h3,10), '-',...
 		'LineWidth',0.5,...
 		'Color',[nhaColor,.3]);
...
function h = plot_curves(hAx, T, yLims)
...
% 	h = gca;
%     ylim(h,[0,0.000001])
% 	h.YAxis(2).Color = [0.15 0.15 0.15];
...
function adjust_positions(T, hSub, hYLab, hLeg, hCol,pWidthMax)
	...
	hSub(1).Position(4) = pHeightMap;
	hSub(2).Position(4) = pHeightCurves;
	...
	hSub(1).Position(2) = pStartY+hSub(2).Position(4)+gap+hSub(3).Position(4)+gap;
...
function add_transition_annotations(hAx, T, segs)
	...
	for i=1:numel(events)
		...
		add_segment_annotation(hAx, segs, isEvent, lower(string(events(i))), 90, [.15 .15 .15])
	end
...
function [hFig, hSub] = init_panels(spec, tit, figWidth, figHeight)
	...
	hSub(1) = subplot(3,1,1,spec.subPlt{:},'FontSize',11, 'FontName','Arial', 'LineWidth',0.74);
	hSub(2) = subplot(3,1,2,spec.subPlt{:},'FontSize',11, 'FontName','Arial', 'LineWidth',0.74);
	hSub(3) = subplot(3,1,3,spec.subPlt{:},'FontSize',11, 'FontName','Arial', 'LineWidth',0.74);

	hSub(3).XAxis.FontSize = 9;
